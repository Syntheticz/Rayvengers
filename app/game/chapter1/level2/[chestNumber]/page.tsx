...
export default function ChestQuestionPageLevel2() {
  ...
  // Debug: lifecycle
  useEffect(() => {
    console.log('[ChestQuestionPage][level2] mount chest', chestNumber);
    return () => console.log('[ChestQuestionPage][level2] unmount chest', chestNumber);
  }, [chestNumber]);
  ...
  const handleElementClick = (elementType: string) => {
    ...
    if (!socket) {
      console.log('[handleElementClick][level2] no socket yet');
    }

    const isCorrect = elementType === questionData.target;
    const questionId = `chest${chestNumber}`;
    console.log('[handleElementClick][level2] attempt', { elementType, isCorrect, attemptsBefore: attempts });

    setAttempts(prev => {
      const updated = prev + 1;
      console.log('[attempts][level2] increment (click)', { prev, updated });
      if (socket) {
        console.log('[socket.emit][level2] submitAnswer', { questionId, answer: elementType, isCorrect, attempts: updated });
  ...
  useEffect(() => {
    console.log('[attempts][level2] state changed ->', attempts);
  }, [attempts]);
  ...
  return (
    <div style={{
      ...
      {/* Back Button */}
      <button
        onClick={() => router.push('/game/chapter1/level2')}
        style={{
          position: "absolute",
          top: "20px",
          left: "20px",

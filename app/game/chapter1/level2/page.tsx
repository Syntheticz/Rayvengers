...
export default function Chapter1Level2() {
  ...
  useEffect(() => {
    ...
    s.on("connect", () => {
      console.log("[game][level2] Connected to game server");
      s.emit("getQuestions");
    });
    ...
    s.on(
      ...
      (data: { questionStates: Record<string, QuestionState> }) => {
        console.log("[game][level2] Question states updated:", data);
    ...
    s.on("answerResult", (data: { questionId: string; isCorrect: boolean }) => {
      console.log("[game][level2] Answer result:", data);
    });
    ...
    s.on('questionsError', (err) => {
      console.log('[game][level2] questionsError', err);
  ...
  const handleClaimQuestion = (index: number) => {
    ...
    router.push(`/game/chapter1/level2/${chestNumber}`);
  };
  ...
  return (
    ...
    >
      ...
      >
        Level 2 – The Light Seeker (Clone)
      </h1>

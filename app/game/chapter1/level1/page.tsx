...
export default function Chapter1Level1() {
  ...
  useEffect(() => {
    ...
    s.on("questionsUpdate", (data: { questions: Question[], questionStates: Record<string, QuestionState> }) => {
  ...
  const handleSubmitAnswer = () => {
    if (!socket || !sessionId || !selectedQuestion || selectedAnswer === null) return;
  ...
  const getQuestionStatusColor = (state: QuestionState | undefined) => {
    ...
    switch (state.status) {
      ...
      case 'completed': return state.isCorrect ? '#22c55e' : '#ef4444';
  ...
  const getQuestionStatusText = (state: QuestionState | undefined) => {
    ...
    switch (state.status) {
      case 'available': return 'Ready to Open';
      case 'claimed': return `Opened by ${state.claimedByName}`;
      case 'completed': return `${state.isCorrect ? 'Treasure Found!' : 'Empty Chest'} by ${state.claimedByName}`;
    ...
    return (
      <div style={{ 
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: "14px" }}>
                  ...
                  <div style={{
                    ...
                    <img 
                      ...
                      style={{
                        ...
                        objectFit: "contain",
                        filter: isCompleted 
                          ? (state?.isCorrect ? "hue-rotate(120deg) saturate(1.2)" : "hue-rotate(0deg) saturate(1.5)")
                          : isClaimed 
                    ...
                    {isAvailable && (
                      <div style={{
                        position: "absolute",
                        inset: "-10px",
                        background: "radial-gradient(circle, rgba(255,204,102,0.3) 0%, transparent 70%)",
                        borderRadius: "50%",
            ...
            <div style={{ display: "grid", gap: "12px" }}>
              {selectedQuestion.options.map((option, index) => (
                <button
                  ...
                  style={{
                    background: selectedAnswer === index ? "#ffcc66" : "#f5f5f5",
                    border: `2px solid ${selectedAnswer === index ? "#b80f2c" : "#ddd"}`,
                    borderRadius: "8px",
                    padding: "12px 16px",
                    cursor: "pointer",
                    fontSize: "14px",
